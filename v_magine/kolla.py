...
class KollaInstaller(object):
    ...
    def _exec_shell_cmd_check_exit_status(self, cmd):
        ...
        while running:
            ...
            if chan.exit_status_ready():
                LOG.info("Inside exit_status_ready")
                running = False
                LOG.info("Past exit_status_ready")
    ...
    def update_os(self):
        ...
        self._exec_shell_cmd_check_exit_status('apt update -y && apt install -y linux-generic-hwe-16.04 && apt dist-upgrade -y && exit 0')
        LOG.info("OS updated")

    def reboot(self):
        LOG.info("Rebooting")
        self._exec_cmd("reboot > /dev/null 2>&1 &")
        LOG.info("Reboot command finished")
        self.disconnect()

    def configure_networking(self):
        networking_script = 'config_networking.sh'
        self._copy_resource_file(networking_script)
        self._exec_shell_cmd_check_exit_status("/bin/chmod u+x /root/%(networking_script)s && "
                                            "/root/%(networking_script)s"
                                            % {"networking_script": networking_script})
        LOG.info("Networking configured")

    def _exec_utils_function(self, cmd):
        ubuntu_utils = "ubuntu-utils.sh"
        self._copy_resource_file(ubuntu_utils)
        return self._exec_cmd(". /root/%(ubuntu_utils)s && %(cmd)s" %
                              {"ubuntu_utils": ubuntu_utils, "cmd": cmd})
    ...
    def install_kolla(self, kolla_admin_password, fip_range, fip_range_start,
        ...
        self._exec_shell_cmd_check_exit_status(
            ...
            {'install_script': install_script,
             ...
             'ext_vswitch_name': ext_vswitch_name,
